...
## Shang Chun, Jul 2022
## PDB为体系搭好后的.psf文件（ionized.psf）
## TRJ为轨迹文件（.dcd）
## REF为参考的结构文件，即体系搭好后的.pdb文件（ionized.pdb）

# if [ ! -n "$REF" ]; then
#     REF="$PDB"
# fi
SELREF=("{segname PROA and name CA} or {segname PROB and name CA} or {segname PROC and name CA}")
SELRMSD=("{segname PROD and name CA} or {segname PROE and name CA}")
