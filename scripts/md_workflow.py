...
使用示例:
  ...
  仅生成SLURM脚本 (需要已有质量检查结果):
    ...
    workflow_group.add_argument(
        "--analysis-only",
        action="store_true",
        help="仅运行轨迹分析 (需要已有PBC处理结果)"
    )
    workflow_group.add_argument(
        "--comparative-only",
        action="store_true",
        help="仅运行对比分析 (需要已有轨迹分析结果)"
    )
    ...
    # 轨迹分析参数
    analysis_group = parser.add_argument_group("轨迹分析参数")
    analysis_group.add_argument(
        "--analysis-types",
        nargs="+",
        choices=["rmsd", "rg", "radius_gyration", "distances", "distance", "rdf", "hbonds"],
    ...
    # 步骤3: 轨迹分析 (仅在 analysis-only 模式时运行)
    if args.analysis_only:
        print("\n🔬 运行轨迹分析...")
        analysis_cmd = [
            sys.executable, "run_trajectory_analysis.py",
            str(md_root),
            "--analysis"] + args.analysis_types + [
            "--max-workers", str(args.analysis_workers)
        ]
        
        if args.verbose:
            analysis_cmd.append("--verbose")
        
        if not run_command(analysis_cmd, "轨迹分析"):
            workflow_success = False
            print("\n❌ 轨迹分析失败")
            return 1
        else:
    ...
    # 步骤4: 对比分析 (仅在 comparative-only 模式时运行)
    if args.comparative_only:
        print("\n📊 运行对比分析...")
        comparative_cmd = [
            sys.executable, "run_comparative_analysis.py",
            str(md_root),
            "--output", str(output_dir / "comparative_analysis")
        ]
        
        if args.verbose:
            comparative_cmd.append("--verbose")
        
        if not run_command(comparative_cmd, "对比分析"):
            workflow_success = False
            print("\n❌ 对比分析失败")
            return 1
        else:
